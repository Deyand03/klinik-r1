...
<div x-data="{
        ...
        @if(session('error'))
            <div class="mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg flex items-start gap-3 shadow-sm animate-pulse">
                <svg class="h-6 w-6 text-red-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                <div>
        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-8">
            <div class="flex flex-col md:flex-row gap-4 items-end">
                ...
                <div class="form-control w-full md:w-48">
                    <label class="label pb-1">
                        <span class="label-text font-bold text-gray-500 text-xs uppercase tracking-wider">Hari Praktek</span>
                    </label>
                ...
                <button @click="searchTerm=''; filterHari=''; filterStatus=''" type="button" class="btn btn-outline border-gray-200 text-gray-600 hover:bg-gray-100 hover:border-gray-300 w-full md:w-auto rounded-xl">
                    Reset
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <div class="flex justify-between items-start pl-2">
                        <div class="flex items-center gap-4">
                            <div class="avatar {{ $jadwal['status'] == 'aktif' ? 'online' : 'offline' }}">
                                <div class="w-14 h-14 rounded-2xl ring-2 ring-offset-2 {{ $jadwal['status'] == 'aktif' ? 'ring-brand-secondary/30' : 'ring-gray-200' }}">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($jadwal['dokter']) }}&background=random&bold=true" />
                                </div>
                            </div>
                            <div class="flex flex-col">
                                <h3 class="font-bold text-lg text-brand-dark group-hover:text-brand-btn transition">{{ $jadwal['dokter'] }}</h3>
                                <h4 class="text-xs text-gray-500 font-medium uppercase tracking-wide">{{ $jadwal['spesialis'] }}</h4>
                            </div>
                        </div>
                        <span class="badge {{ $jadwal['status'] == 'aktif' ? 'bg-green-100 text-green-700 border-none' : 'bg-gray-100 text-gray-500 border-none' }} font-bold text-xs py-3 px-3">
                        ...
                        <div class="flex justify-between items-start">
                            ...
                            <div class="flex flex-wrap justify-end gap-1.5 max-w-[70%]">
                                @foreach ($jadwal['hari'] as $item)
                                    <span class="px-2 py-1 rounded-md text-[10px] font-bold border {{ $jadwal['status'] == 'aktif' ? 'bg-white border-brand-secondary/30 text-brand-secondary' : 'bg-white border-gray-200 text-gray-400' }}">
                                        {{ substr($item, 0, 3) }}
                                    </span>
                                @endforeach
                            </div>
                    ...
                    <div class="pl-1 pt-5">
                        <div class="flex justify-between text-xs mb-1.5">
                            ...
                            <span class="font-bold {{ $jadwal['terisi'] >= $jadwal['kuota'] ? 'text-red-500' : 'text-brand-secondary' }}">
                                {{ $jadwal['terisi'] }} <span class="text-gray-400 font-normal">/ {{ $jadwal['kuota'] }}</span>
                            </span>
                        </div>
                        <progress class="progress w-full h-2 {{ $jadwal['terisi'] >= $jadwal['kuota'] ? 'progress-error' : ($jadwal['status'] == 'aktif' ? 'progress-success' : 'progress-secondary') }}"
            ...
            @empty
                <div class="col-span-1 md:col-span-2 xl:col-span-3 text-center py-10 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
        ...
        <div x-show="editModalOpen" style="display: none;"
            ...
            <div x-show="editModalOpen"
                 ...
                 class="fixed inset-0 bg-brand-dark/60 backdrop-blur-sm transition-opacity"
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div x-show="editModalOpen"
                    ...
                    <div class="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                        ...
                        <button @click="editModalOpen = false" class="text-gray-400 hover:text-red-500 transition p-2 hover:bg-red-50 rounded-full">
                        ...
                        <form method="POST" :action="updateUrlTemplate.replace('999', selectedJadwal?.id)">
                            ...
                            <div class="grid grid-cols-2 gap-5 mb-5">
                                <div>
                                    ...
                                    <select name="status" class="select select-bordered w-full bg-white focus:border-brand-secondary rounded-xl transition-all" :value="selectedJadwal?.status">
                                        <option value="aktif">Aktif</option>
                                        <option value="nonaktif">Non-Aktif</option>
                                    </select>
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Kuota Harian</label>
                                    <input type="number" name="kuota" :value="selectedJadwal?.kuota" class="input input-bordered w-full bg-white focus:border-brand-secondary rounded-xl transition-all">
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-5 mb-6">
                                <div>
                                    ...
                                    <input type="time" name="jam_mulai" :value="selectedJadwal?.jam_mulai" class="input input-bordered w-full bg-white focus:border-brand-secondary rounded-xl transition-all">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Jam Selesai</label>
                                    <input type="time" name="jam_selesai" :value="selectedJadwal?.jam_selesai" class="input input-bordered w-full bg-white focus:border-brand-secondary rounded-xl transition-all">
                                </div>
                            </div>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach(['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'] as $day)
                                        <label class="cursor-pointer relative">
                                            ...
                                            <div class="px-4 py-2 rounded-xl text-sm font-semibold border border-gray-200 text-gray-500 bg-white transition-all
                            ...
                            <div class="mt-8 flex flex-row-reverse gap-3">
                                <button type="submit" class="btn bg-brand-secondary hover:bg-brand-secondary/90 text-white border-none px-6 rounded-xl shadow-lg transform hover:-translate-y-0.5 transition-all">
                                    Simpan Perubahan
                                </button>
                                <button @click="editModalOpen = false" type="button" class="btn btn-ghost text-gray-500 hover:bg-gray-200 rounded-xl">
                                    Batal
                                </button>
                            </div>
                        </form>
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div x-show="addModalOpen"
                    ...
                    <div class="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                        ...
                        <button @click="addModalOpen = false" class="text-gray-400 hover:text-red-500 transition p-2 hover:bg-red-50 rounded-full">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                    </div>
                    ...
                    <div class="px-8 py-6">
                        <form action="{{ route('admin.jadwal-dokter.store') }}" method="POST">
                            ...
                            <div class="mb-5">
                                ...
                                <select name="staff_id" class="select select-bordered w-full bg-white focus:border-brand-secondary focus:ring-2 focus:ring-brand-secondary/20 rounded-xl transition-all">
                            ...
                            <div class="grid grid-cols-2 gap-5 mb-5">
                                <div>
                                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Status Awal</label>
                                    <select name="status" class="select select-bordered w-full bg-white focus:border-brand-secondary rounded-xl transition-all">
                                ...
                                <div>
                                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Kuota Harian</label>
                                    <input type="number" name="kuota" value="20" class="input input-bordered w-full bg-white focus:border-brand-secondary rounded-xl transition-all">
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-5 mb-6">
                                <div>
                                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Jam Mulai</label>
                                    <input type="time" name="jam_mulai" class="input input-bordered w-full bg-white focus:border-brand-secondary rounded-xl transition-all">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Jam Selesai</label>
                                    <input type="time" name="jam_selesai" class="input input-bordered w-full bg-white focus:border-brand-secondary rounded-xl transition-all">
                                </div>
                            </div>
                            ...
                            <div class="mt-8 flex flex-row-reverse gap-3">
                                ...
                                <button @click="addModalOpen = false" type="button" class="btn btn-ghost text-gray-500 hover:bg-gray-200 rounded-xl">
                                    Batal
                                </button>
                            </div>
                        </form>
